...
- **Story 1.4 (NEW):** Implement a thread-safe database session context manager (`get_db_session()`) to prevent SQLite thread locks during background execution in Streamlit.
...
- **Story 2.4 (NEW):** Ensure graph execution handles chapter state transitions explicitly (`current_chapter_number`, `raw_text`, `polished_text`).
...
- **Story 3.4.1 (Scene Writer - FIXED):** Takes `beat_outline`, drafts markdown prose, and explicitly creates/updates a record in the SQLite `Chapter` table with `status="draft"`.
- **Story 3.6.1 (Prose Stylist - FIXED):** Polishes chapter text, updates the SQLite `Chapter` record with `polished_text`, and marks `status="completed"`.
...
- **Story 4.4.1 (Manuscript Reader - FIXED):** Queries SQLite `Chapter` table by `novel_id`, rendering both `raw_text` and `polished_text` alongside Red Team audit notes in markdown format.
